...
def QAGeneration(topic: str):
    query = f"Give me 15 symptoms of {topic} skin disease , each seperated on a new line, don't say anything more than that"
    res = LLM.generator(query, vectorDB, llm)
    return res


def imageAnalysis(imgList):
    predictionIndex = CNN.makePrediction(imgList, imageModel)
